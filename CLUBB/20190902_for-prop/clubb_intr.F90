...
!--CMZ
    ...
    !call pbuf_add_field('SCLRPTHVP',  'global', dtype_r8, (/pcols,pverp,sclr_dim,dyn_time_lvls/), sclrpthvp_idx)
    ...
    !call pbuf_add_field('PDF_PARAMS', 'global', dtype_r8, (/pcols,pverp,num_pdf_params,dyn_time_lvls/), pdf_params_idx)
    !call pbuf_add_field('PDF_PARAMS_ZM', 'global', dtype_r8, (/pcols,pverp,num_pdf_params,dyn_time_lvls/), pdf_params_zm_idx)
...
!$OMP PARALLEL
    ...
    if ( err_code == clubb_fatal_error ) then
        print *, "FATAL ERROR CALLING SETUP_CLUBB_CORE"
    end if
!$OMP END PARALLEL
...
!   use cam_abortutils, only: endrun
   ...
   !  advance_clubb_core_module, only: advance_clubb_core
   ...
   real(r8) :: zt_out(pcols,pverp+1-top_lev)              ! output for the thermo CLUBB grid           	[m] 
   real(r8) :: zi_out(pcols,pverp+1-top_lev)              ! output for momentum CLUBB grid             	[m]
...
!   real(r8), pointer, dimension(:,:,:) :: sclrpthvp   ! sclr'th_v' (momentum levels)		[{units vary} K]
...
!   call pbuf_get_field(pbuf, sclrpthvp_idx, sclrpthvp, start=(/1,1,1,itim_old/), kount=(/pcols,pverp,sclr_dim,1/))
   !call pbuf_get_field(pbuf, pdf_params_idx, pdf_params_ptr, start=(/1,1,1,itim_old/), kount=(/pcols,pverp,num_pdf_params,1/))
   !call pbuf_get_field(pbuf, pdf_params_zm_idx, pdf_params_zm_ptr, start=(/1,1,1,itim_old/), kount=(/pcols,pverp,num_pdf_params,1/))
...
!           These lines were incoming on a merge; I ignored them.
!           Eric Raut Aug 2015
...
!--CMZ
       ...
       if ( err_code == clubb_fatal_error ) then
           print *, "FATAL ERROR IN CLUBB: at timestep ", get_nstep()
           print *, "LAT: ", state1%lat(i), " LON: ", state1%lon(i)
           stop
       end if
...
!         sclrthvp(i,k,:)   = sclrpthvp_inout(pverp-k+1,:)
...
!     accre_enhan(:ncol,:pver) = 1._r8+0.65_r8*(1.0_r8/relvar(:ncol,:pver))
   ...
   do k = top_lev, pver
      ...
      call aist_vector(state1%q(:,k,ixq),state1%t(:,k),state1%pmid(:,k),state1%q(:,k,ixcldice), &
           ...
           ! qsatfac_out=qsatfac(:,k), rhmini_in=rhmini, rhmaxi_in=rhmaxi) ! Uncomment this line to reproduce trunk (ZM, no-SILHS) results
   enddo
   ...
   rrho = (1._r8/gravit)*(state1%pdel(1:ncol,pver)/dz_g(pver)) 
