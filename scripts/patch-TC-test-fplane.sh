...
if [ "1" -eq "0" ]; then
  ...
  filename="shr_flux_mod"
  SUBPATH="cime/src/share/util/"
  CESMCOMPONENT="share"
  SOURCEMODDIR=~/ideal-mods/CLUBB/SourceDiffs/src.${CESMCOMPONENT}/
  mkdir -p ${SOURCEMODDIR}
  diff -b -U 3 ${CESMROOT}/${SUBPATH}/${filename}.F90 ~/TC_PBL/RCE.QPC6.ne0np4tcfplane.ne15x8.exp297.001/SourceMods/src.${CESMCOMPONENT}/${filename}.F90 > ${SOURCEMODDIR}/${filename}.patch
fi
...
CAM5_DO_MG2=true
## General settings
CD_CAP_MS=31.0         # negative means no cap
...
if [ ${PHYS} == 'QPC6' ] ; then
  PATCHDIR=${IDEALDIR}/CLUBB/SourceDiffs/
  #patch_code ${PATCHDIR} "advance_clubb_core_module" "components/cam/src/physics/clubb/" "cam"
  #patch_code ${PATCHDIR} "clubb_api_module" "components/cam/src/physics/clubb/" "cam"
  #patch_code ${PATCHDIR} "clubb_intr" "components/cam/src/physics/cam/" "cam"
...
if [ ${PHYS} == 'QPC5' ] ; then
  ...
  EXTRAVARS="'Lscale','wp2','C11_Skw_fnc'"
  DIAGOUT=","$EXTRAVARS
else
  DIAGOUT=""
fi
